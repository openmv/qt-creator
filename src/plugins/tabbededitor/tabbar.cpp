...
void TabBar::addEditorTab(Core::IEditor *editor)
{
    ...
    if(path.isEmpty())
    {
        setTabIcon(index, Core::FileIconProvider::icon(QFileInfo(Core::ICore::userResourcePath() + QStringLiteral("/examples/OpenMV/01-Basics/helloworld.py"))));
...
void TabBar::closeTab(int index)
{
    ...
    if(Core::EditorManager::instance()->closeEditors(QList<Core::IEditor *>() << m_editors.at(index)))
    {
        m_editors.removeAt(index);
        removeTab(index);
    }
