    ...
    connect(manager, &QNetworkAccessManager::finished, this, [this, manager] (QNetworkReply *reply) {
        ...
        if((reply->error() == QNetworkReply::NoError) && (!data.isEmpty()))
        {
            ...
            if(match.hasMatch())
            {
                ...
                {
                    QMessageBox box(QMessageBox::Information, tr("Update Available"), tr("New OpenMV IDE resources are available (e.g. examples, firmware, documentation, etc.)."), QMessageBox::Cancel, Core::ICore::dialogParent(),
        ...
        if(forceBootloader)
        {
            ...
            if(isIMX)
            {
                ...
                else
                {
                    QMessageBox::critical(Core::ICore::dialogParent(),
                        tr("Connect"),
                        tr("Error: %L1!").arg(imxSettings.errorString()));
                    ...
                    if(userCanceled)
                    {
                        ...
                        == QMessageBox::Ok)
                        {
                            if(QMessageBox::information(Core::ICore::dialogParent(),
                                tr("Connect"),
                                tr("Disconnect your OpenMV Cam from your computer, add a jumper wire between the SBL and 3.3V pins, and then reconnect your OpenMV Cam to your computer.\n\n"
            ...
            if(isArduino)
            {
                ...
                if(isPortenta || isNiclav)
                {
                    ...
                    if(forceFlashFSErase)
                    {
                        ...
                        if(file.open())
                        {
                            if(file.write(QByteArray(FLASH_SECTOR_ERASE, 0)) == FLASH_SECTOR_ERASE)
                            {
                                ...
                                {
                                    downloadFirmware(command, process, QFileInfo(file).canonicalFilePath(), dfuDeviceVidPid, eraseCommands.at(i) + ((justEraseFlashFs && ((i + 1) == j)) ? QStringLiteral(":leave") : QStringLiteral("")) + dfuDeviceSerial);
                    ...
                    // Program Flash //////////////////////////////////////
                    {
                        ...
                        QString command;
                        Utils::QtcProcess process;
                        downloadFirmware(command, process, QDir::toNativeSeparators(QDir::cleanPath(firmwarePath)), dfuDeviceVidPid, (firmwarePath.endsWith(QStringLiteral(".bin"), Qt::CaseInsensitive) ? binProgramCommand : dfuProgramCommand) + dfuDeviceSerial);
                ...
                else if(isNRF)
                {
                    ...
                    QString command;
                    Utils::QtcProcess process;
                    bossacDownloadFirmware(command, process, QDir::toNativeSeparators(QDir::cleanPath(firmwarePath)), dfuDevicePort, binProgramCommand);
                ...
                else if(isRPIPico)
                {
                    ...
                    if(forceFlashFSErase)
                    {
                        ...
                        if(file.open())
                        {
                            if(file.write(QByteArray(FLASH_SECTOR_ERASE, 0)) == FLASH_SECTOR_ERASE)
                            {
                                ...
                                for(int i = 0, j = eraseCommands.size(); i < j; i++)
                                {
                                    picotoolDownloadFirmware(command, process, QFileInfo(file).canonicalFilePath(), eraseCommands.at(i));
                    ...
                    // Program Flash //////////////////////////////////////
                    {
                        QString command;
                        Utils::QtcProcess process;
                        picotoolDownloadFirmware(command, process, QDir::toNativeSeparators(QDir::cleanPath(firmwarePath)), binProgramCommand);
