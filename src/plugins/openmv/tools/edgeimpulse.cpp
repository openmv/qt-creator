...
static void uploadProject(const QString &apiKey, const QString &hmacKey, OpenMVDatasetEditor *editor)
{
    ...
    if(ok)
    {
        ...
        for(const QString &className : editor->classFolderList())
        {
            for(const QString &snapshotName : editor->snapshotList(className))
            {
    ...
    QObject::connect(manager, &QNetworkAccessManager::finished, [manager, editor] (QNetworkReply *reply) {
        ...
        if((reply->error() == QNetworkReply::NoError) && (!data.isEmpty()))
        {
            ...
            if(success)
            {
                ...
                QJsonArray temp = body.value(QStringLiteral("projects")).toArray(); for(const QJsonValue project : temp)
                {
